...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	flaminginsulation: {
		...
		onTry(source, target, move) {
			...
			if (hpThresh>=0.67 && !target.volatiles['substitute'] && !target.volatiles['protect'] && !target.volatiles['spikyshield'] && !target.side.getSideCondition('matblock')) {
	...
	mindspike: {
		...
		basePowerCallback(pokemon, target, move) {
			// You can't get here unless the pursuit succeeds
			if (target.beingCalledBack || target.switchFlag) {
				this.debug('Mindspike damage boost');
				return move.basePower * 2;
			}
			return move.basePower;
		},
		...
		beforeTurnCallback(pokemon) {
			for (const target of pokemon.foes()) {
				...
				const data = target.volatiles['mindspike'];
				if (!data.sources) {
					data.sources = [];
				}
				data.sources.push(pokemon);
		...
		condition: {
			duration: 1,
			onBeforeSwitchOut(pokemon) {
				this.debug('Mindspike start');
				let alreadyAdded = false;
				pokemon.removeVolatile('destinybond');
				for (const source of this.effectState.sources) {
					if (!source.isAdjacent(pokemon) || !this.queue.cancelMove(source) || !source.hp) continue;
					if (!alreadyAdded) {
						this.add('-activate', pokemon, 'move: Mindspike');
						alreadyAdded = true;
					}
					// Run through each action in queue to check if the Pursuit user is supposed to Mega Evolve this turn.
					// If it is, then Mega Evolve before moving.
					...
					this.actions.runMove('mindspike', source, source.getLocOf(pokemon));
				}
			},
		},
		target: "normal",
